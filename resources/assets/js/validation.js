
/**
 * First we will load all of this project's JavaScript dependencies which
 * includes Vue and other libraries. It is a great starting point when
 * building robust, powerful web applications using Vue and Laravel.
 */

require('./bootstrap');

import Vue from 'vue'; //Imports Vue framework into the sau-app
import BootstrapVue from 'bootstrap-vue/dist/bootstrap-vue';

window.Vue = Vue; // Adds the vue into the window instance
Vue.use(BootstrapVue);

/**
 * Add sau-app components
 */
Vue.component('sau-form', require('./components/Create'));
Vue.component('sau-validation', require('./components/Validation'));
Vue.component('sau-input', require('./components/helpers/forms/SauInput'));
Vue.component('sau-error', require('./components/helpers/forms/SauError'));

/**
 * Next, we will create a fresh Vue application instance and attach it to
 * the page. Then, you may begin adding components to this application
 * or customize the JavaScript scaffolding to fit your unique needs.
 */
const app = new Vue({
    el: '#validation'
});
